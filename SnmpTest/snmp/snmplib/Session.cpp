...
void Session::sendRequestGetValues( const QStringList& names ) {
    ...
    if( -1 == m_request_id ) {
        ...
        full_packet.addChild( QtSnmpData::integer( 1 ) );//snmp version snmpv1:0 snmpv2:1
...
void Session::sendRequestGetNextValue( const QString& name ) {
    ...
    if( -1 == m_request_id ) {
        updateRequestId();
        QtSnmpData full_packet = QtSnmpData::sequence();
        full_packet.addChild( QtSnmpData::integer( 0 ) );//snmp version snmpv1:0 snmpv2:1
        full_packet.addChild( QtSnmpData::string( m_community ) );
                                   ...
                                   const QByteArray& value )
{
    ...
    if( -1 == m_request_id ) {
        ...
        pdu_packet.addChild( QtSnmpData::integer( 0 ) );//snmp version snmpv1:0 snmpv2:1
