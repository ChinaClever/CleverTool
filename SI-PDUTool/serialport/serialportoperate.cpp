...
void SerialportOperate::enablePort(QString &portName,qint32 baudRate)
{
    ...
    if(!mList.at(ret)->isOpen())
    {
        ...
        mList.at(ret)->setBaudRate(QSerialPort::Baud9600);  //波特率
...
QByteArray SerialportOperate::readDataFromPort(QString &portName)
{
    ...
    {
        while (mList.at(id)->waitForReadyRead(1)); // 等待串口接收完全
...
//--------------------------------------------------------------------------
/*
void SerialportOperate::openPor(QSerialPort *port, int value)
{
    mCPortSun = value;

}

void SerialportOperate::stopPor(QSerialPort *port, int value)
{

}


void SerialportOperate::onGetData()
{
